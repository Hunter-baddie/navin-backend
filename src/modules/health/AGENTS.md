# health Module — AI Agent Context
...
| File | Purpose |
|------|---------|
| `health.routes.ts` | Express router wiring: auth → role → Zod → controller |
| `health.controller.ts` | Thin handlers: extract req, call service, `sendResponse()` |
| `health.service.ts` | Business logic: throw `AppError`, return plain objects |
| `health.model.ts` | Mongoose schema: `isoDatePlugin`, `deletedAt`, soft-delete pre-hooks |
| `health.validation.ts` | Zod schemas + exported inferred types |
| `health.test.ts` | Jest tests: 200 · 401 · 403 · 400 |
