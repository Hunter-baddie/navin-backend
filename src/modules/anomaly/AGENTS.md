# anomaly Module — AI Agent Context
...
| File | Purpose |
|------|---------|
| `anomaly.routes.ts` | Express router wiring: auth → role → Zod → controller |
| `anomaly.controller.ts` | Thin handlers: extract req, call service, `sendResponse()` |
| `anomaly.service.ts` | Business logic: throw `AppError`, return plain objects |
| `anomaly.model.ts` | Mongoose schema: `isoDatePlugin`, `deletedAt`, soft-delete pre-hooks |
| `anomaly.validation.ts` | Zod schemas + exported inferred types |
| `anomaly.test.ts` | Jest tests: 200 · 401 · 403 · 400 |
