...
1. **Copy the template** — Duplicate `src/modules/__template__/` to `src/modules/<domain>/` and rename all `template*` files to `<domain>*`.
2. **Read** root `AGENTS.md` § Architecture Boundaries to confirm your module's allowed dependencies.
3. **Adapt the template** — Replace all placeholder names, types, and business rules with the new domain's concepts.
...
6. **Create tests** — Write `tests/<domain>.test.ts` covering 200 · 401 · 403 · 400.
7. **Swagger** — Add all new paths to `docs/swagger.yaml`.
8. **Create module AGENTS.md** — Write `src/modules/<domain>/AGENTS.md` with the route prefix, file list, error codes, and dependencies.
9. **Update root AGENTS.md** — Add the module to § Architecture Boundaries (even if it has no cross-module deps, list it as "None").
