...
6. **Mock externals** — Use `jest.unstable_mockModule` for Stellar, email, storage, etc. See `tests/helpers/mocks.ts`.
7. **Run tests** — Execute `npm test -- <test-file>` and fix failures before finishing.
8. **Update docs** — If the test reveals a missing or incorrect convention, update the module's `AGENTS.md`.
